...
| A plain focus click always ran `_autosave` → `touch()` → full `rebuild_indexes()` (and a revision-driven `SceneIndex` rebuild), then `_build_focus_code` rebuilt a throwaway name→Focus map with `build_focus_name_lookup`, so selecting one focus at Load All Trees scale paid several O(F+E)/O(F) passes even when the form was untouched | `hoi4_content_maker.py` (`_autosave`, `_select`, `_build_focus_code`), `models/document.py` (`touch`/`rebuild_indexes`, new `by_name`) | `_autosave` compares the form to the selected focus and returns without writing or touching when nothing changed; a real name change still `touch()`es, while x/y keeps using incremental `move()`. Code-tab render takes `focuses.by_name`, an O(1) view over `first_by_name`, instead of scanning every focus | fixed in issue #25 | not yet measured, needs a display session against the real mod |
