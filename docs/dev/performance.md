...
| Export and Save All generated focus scripts, read localisation, and wrote every tree synchronously on the Tk thread. A full Millennium Dawn batch freezes the window for hundreds of writes | `focus_tree/export_plan.py`, `hoi4_content_maker.py:_run_export_plans` | Tk shells resolve paths and snapshot plans, then one worker renders and atomically writes each tree with modal progress. Results, dialogs, and catalog notifications return to the Tk thread | fixed in issue #27 | not yet measured, needs a display session against the real mod |
...
| A malformed file with deeply nested `focus = {` blocks made both raw-reward extraction and the robust fallback rescan the same suffix once per block, so an 8,000-block input could turn one parse into quadratic work | `focus_tree/parse.py` (`_RawRescanBudget`, `parse_focus_tree`) | Share a 4,096-scan / 8,000,000-character budget across every raw-reward and fallback brace scan. This preserves raw fields for the documented largest real tree (776 focuses / 40,790 lines) while bounding the 8,000 nested-block case. Exhausting the budget rejects the current parse instead of returning a partial model, so no raw fields are silently dropped. Poll an optional event/callback throughout both passes; the batch loader drops a cancelled file instead of returning partial model data | fixed in issue #159 | regression tests use 8,000 nested blocks and forced raw/fallback budget exhaustion to assert bounded scans and parse rejection; no broad timing assertion |
...
|---|---|---|---|
| `WorkspaceCache` graphics snapshot (SQLite, `~/.hoi4cm/scan_cache/<hash>.db`) | relative graphics inventory, sprite declarations, and configured focus/idea views | root identity, graphics path fingerprint, directory `mtime_ns`/`ctime_ns`, `.gfx` file stamps, and image stamps | one snapshot per graphics path configuration; detected stale rows rebuild. A warm load restats the known images, so an in-place image overwrite (same filename) is caught without an explicit refresh. Validation returns on the first stale directory, `.gfx`, or image stamp, so a miss does not finish the warm-cache stat pass before rescanning |
