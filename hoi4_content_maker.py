...
def open_national_spirit_wizard(app):
    ...
    def _set_cat_menu(cats, on_select_cmd):
        ...
        menu.delete(0, "end")
        for cat in cats:
            menu.add_radiobutton(label=cat, variable=_mc, value=cat,
    ...
    def _filter_mod_dd(*_):
        ...
        _set_cat_menu(matching_cats or MODIFIER_CATS,
                      lambda c: [_mc.set(c)])
        ...
        menu2 = om["menu"]; menu2.delete(0, "end")
        for k, v in matches:
            menu2.add_command(
    ...
    def _build_output():
        ...
        out.append("")
        out.append("# ============================================================")
        out.append("# LOCALISATION  localisation/english/%s_l_english.yml" % sid)
        out.append("# ============================================================")
        out.append("")
        out.append(' %s: "%s"' % (sid, loc_n))
        out.append(' %s_desc: "%s"' % (sid, loc_d))
        return "\n".join(out)
    ...
    def _save_to_mod():
        ...
        try:
            ...
            if file_exists:
                with open(ideas_path, "r", encoding="utf-8-sig", errors="replace") as f:
                    existing = f.read()
        ...
        if MOD.edit_scripted_loc_file:
            ...
            append_scripted_loc(MOD.edit_scripted_loc_file,
    ...
    def _browse_existing_spirits():
        ...
        if not MOD.loaded or not MOD.root:
            messagebox.showinfo("No Mod Loaded",
                "Load a mod first to browse existing spirits.", parent=win)
            return

        ideas_dir = os.path.join(MOD.root, "common", "ideas")
        if not os.path.isdir(ideas_dir):
            messagebox.showinfo("Not Found",
                "No common/ideas/ directory found in mod.", parent=win)
            return
        ...
        sv_q = tk.StringVar()
        tk.Entry(dlg, textvariable=sv_q, bg=BG_CARD, fg=TEXT,
        ...
        lb = tk.Listbox(frm_list, bg=BG_CARD, fg=TEXT, selectbackground=SEL_BG,
                        selectforeground=TEXT, font=("Courier",9), relief="flat",
                        highlightthickness=1, highlightbackground=BORDER_G,
                        activestyle="none")
        ...
        def _filter_list(*_):
            ...
            filtered = [(sid, sk, fp) for sid, sk, fp in spirits
                        if not q or q in sid.lower() or q in sk.lower()
        ...
        info_lbl = tk.Label(dlg, text=f"{len(spirits)} spirits found in mod",
        ...
        def _load_selected():
            ...
            try:
                ...
                spirit = slot_data.get(spirit_id, {}) if isinstance(slot_data, dict) else {}
                if not isinstance(spirit, dict):
                    messagebox.showerror("Parse Error",
                        f"Could not parse spirit '{spirit_id}'", parent=dlg)
                    return
            except Exception as e:
...
def open_decision_wizard(app):
                    ...
                    if dec_icon_key:
                        def _async_dec_icon(key=dec_icon_key, box=icon_box,
                            ...
                            def _worker():
                                ...
                                _path = (MOD.decision_sprites.get(key)
                                         or MOD.idea_sprites.get(key)
                                         or MOD.sprites.get(key))
                                if not _path:
                                    _stem = key
                                    for _pfx in ("GFX_decision_category_","GFX_decision_","GFX_idea_","GFX_"):
                                        if key.startswith(_pfx): _stem = key[len(_pfx):]; break
                                    if MOD.root:
                                        for _sub in ("gfx/interface/decisions","gfx/interface/decisions/categories",
                                                     "gfx/interface","gfx/interface/ideas"):
                                            for _ext in (".dds",".tga",".png"):
                                                _p = os.path.join(MOD.root, _sub.replace("/",os.sep), _stem+_ext)
                                                if os.path.isfile(_p): _path = _p; break
                                            if _path: break
                                        if not _path:
                                            _dr = os.path.join(MOD.root,"gfx","interface","decisions")
                                            if os.path.isdir(_dr):
                                                for _rd,_,_fs in os.walk(_dr):
                                                    for _ext in (".dds",".tga",".png"):
                                                        _p = os.path.join(_rd, _stem+_ext)
                                                        if os.path.isfile(_p): _path = _p; break
                                                    if _path: break
                                if not _path: return
                                # Load PIL image in background (safe), create PhotoImage on main thread
                                try:
                                    _pil = _PILImage.open(_path).convert("RGBA")
                                    _rs  = getattr(_PILImage,"LANCZOS",getattr(_PILImage,"ANTIALIAS",1))
                                    _pil = _pil.resize((24,24), _rs)
                                except Exception: return
                                def _paint():
                                    if not box.winfo_exists(): return
                                    try:
                                        img = _PILImageTk.PhotoImage(_pil)
    ...
    def _browse_mod_decisions():
        ...
        lb = tk.Listbox(frm, bg=BG_CARD, fg=TEXT, selectbackground=SEL_BG,
                        selectforeground=TEXT, font=("Courier",10), relief="flat",
                        highlightthickness=1, highlightbackground=BORDER_G,
            ...
            for fp in selected:
                try:
                    ...
                    for m in _re2.finditer(r'\b([A-Za-z_][A-Za-z0-9_]*)\s*=\s*\{', raw):
                        cid = m.group(1)
                        if cid not in ("add_namespace", "namespace") and cid in existing_cat_ids:
                            new_cats.append(cid)
                except Exception:
                    pass
            if new_cats:
    ...
    def _save_to_mod():
        ...
        try:
            ...
            if os.path.isfile(yml_path):
                with open(yml_path,"r",encoding="utf-8-sig",errors="replace") as f:
                    for line in f:
                        m = _re_loc2.match(r'\s+(\S+?)(?::\d+)?\s*[=:]?\s*"', line)
                        if m: existing_loc_keys.add(m.group(1))
            else:
            ...
            for ln in new_lines:
                ...
                if m and m.group(1) in existing_loc_keys: continue
                to_write.append(ln)
            if to_write:
                with open(yml_path,"a",encoding="utf-8-sig") as f: f.write("\n".join(to_write)+"\n")
            saved.append(os.path.relpath(yml_path,mod_root) + f"  (+{len(to_write)} keys)")
        except Exception as e: errs.append(str(e))
        # ── SCRIPTED LOC ─────────────────────────────────────────────────
        if MOD.edit_scripted_loc_file:
            sloc_blocks = []
...
def open_dyn_mod_wizard(app):
    ...
    def _build_output():
        ...
        dm += ["",
               ...
               f"\t\t\t# Use adds_dynamic_modifier_tt if this focus also calls add_dynamic_modifier",
               f"\t\t\t# Use modifies_dynamic_modifier_tt if only changing variables (no add_dynamic_modifier)",
        ...
        dm += ["",
               ...
               f" {mid}: \"{name}\"",
               f" {mid}_desc: \"{desc}\"",
               f" modifies_dynamic_modifier_tt: \"Modifies $MODIFIER$\"",
               f" adds_dynamic_modifier_tt: \"Adds $MODIFIER$ which grants:\"  # for activation focuses"]
        if mod_entries:
            dm.append("")
    ...
    def _save_file():
        ...
        snippet  = [
            ...
            f"# ============================================================",
            f"",
            f"# ── ACTIVATION focus (first time adding this modifier) ────────",
            f"# Use adds_dynamic_modifier_tt when the block contains add_dynamic_modifier",
            f"\t\t\tadd_dynamic_modifier = {{ modifier = {mid} }}",
            f"\t\t\tcustom_effect_tooltip = {{",
            f"\t\t\t\tlocalization_key = adds_dynamic_modifier_tt",
            f"\t\t\t\tMODIFIER = {mid}",
            f"\t\t\t}}",
            f"",
            f"# ── MODIFICATION focus (changing variables only, no add_dynamic_modifier) ─",
            f"# Use modifies_dynamic_modifier_tt when only variables are changed",
            f"\t\t\tcustom_effect_tooltip = {{",
            f"\t\t\t\tlocalization_key = modifies_dynamic_modifier_tt",
            f"\t\t\t\tMODIFIER = {mid}",
            f"\t\t\t}}",
    ...
    def _browse_mod_dynmods():
        ...
        if not MOD.loaded or not MOD.root:
            messagebox.showinfo("No Mod Loaded",
                "Load a mod first to browse existing dynamic modifiers.", parent=win)
            return
        dm_dir = os.path.join(MOD.root, "common", "dynamic_modifiers")
        if not os.path.isdir(dm_dir):
            messagebox.showinfo("Not Found",
                "No common/dynamic_modifiers/ directory found in mod.", parent=win)
            return
        ...
        if not mods:
            messagebox.showinfo("No Modifiers Found",
                "No dynamic modifier definitions found.", parent=win)
            return
        ...
        def _load_selected():
            ...
            try:
                ...
                if not isinstance(blk, dict):
                    messagebox.showerror("Parse Error",
                        f"Could not parse modifier '{modifier_id}'", parent=dlg)
                    return
            except Exception as e:
                messagebox.showerror("Parse Error", str(e), parent=dlg)
                return
            ...
            _preview()
            dlg.destroy()
            app._hint(f"Loaded dynamic modifier '{modifier_id}' from {os.path.basename(fp)}")
...
def open_additional_income_wizard(app):
    ...
    tk.Button(hdr, text="✕", command=win.destroy,
              bg="#0d2b1a", fg=TEXT_DIM, relief="flat",
              font=("Helvetica", 10), cursor="hand2").pack(side="right", padx=10)
    ...
    def _lbl(parent, text, bold=False, dim=False):
        ...
        tk.Label(parent, text=text, bg=BG_PANEL, fg=fg, font=font, anchor="w").pack(fill="x", pady=(6,1))
    ...
    def _field(parent, label, default, hint="", width=36):
        ...
        if hint:
            tk.Label(row, text=hint, bg=BG_PANEL, fg=TEXT_DIM,
    ...
    # ── Section card ─────────────────────────────────────────────────────
    card = tk.Frame(body_outer, bg=BG_PANEL,
                    highlightthickness=1, highlightbackground=BORDER_G)
    ...
    hint_lbl = tk.Label(inner, text="", bg=BG_PANEL, fg=TEXT_DIM, font=("Helvetica", 8), anchor="w")
    ...
    def _refresh_status():
        ...
        lines.append(f"  00_money_system.txt       {'✅ found' if MOD.md_money_system_file else '❌ not found'}")
        lines.append(f"  money_scripted_loc.txt    {'✅ found  — ' + os.path.basename(MOD.md_money_scripted_loc_file) if MOD.md_money_scripted_loc_file else '❌ not found (will create)'}")
        lines.append(f"  MD_money_l_english.yml    {'✅ found' if MOD.md_money_yml_file else '❌ not found (will create)'}")
    ...
    # ── Results area ──────────────────────────────────────────────────────
    result_frm = tk.Frame(inner, bg="#050810",
                          highlightthickness=1, highlightbackground=BORDER_G)
    result_frm.pack(fill="both", expand=True, pady=(6,0))
    result_txt = tk.Text(result_frm, bg="#050810", fg="#8aad8a",
    ...
    def _apply():
        ...
        saved, errs = app._apply_md_additional_income(idea_id, variable_name, amount, tooltip_key,
        ...
        if tooltip_text and tooltip_key:
            loc_path = MOD.edit_loc_file if MOD.edit_loc_file and os.path.isfile(MOD.edit_loc_file) \
                       else os.path.join(MOD.root, "localisation", "english", f"{idea_id}_l_english.yml")
            try:
                existing_keys = set()
                if os.path.isfile(loc_path):
                    with open(loc_path, "r", encoding="utf-8-sig", errors="replace") as fp:
                        for line in fp:
                ...
                if tooltip_key not in existing_keys:
                    to_write[tooltip_key] = f"$$[?{variable_name}|+3] from §Y${idea_id}$§!\\n"
                if not os.path.isfile(loc_path):
                ...
                if to_write:
                    with open(loc_path, "a", encoding="utf-8-sig") as fp:
                        for k, v in to_write.items():
                            fp.write(f' {k}: "{v}"\n')
                    output_lines.append(f"✅ {os.path.relpath(loc_path, MOD.root)}  — wrote tooltip localisation")
            except Exception as e:
        ...
        if mode == "also_spirit":
            output_lines.append("")
            output_lines.append("── Spirit Code Snippet (paste into your ideas file) ──")
            snippet = (
                f"\t{idea_id} = {{\n"
                f"\t\tname = {idea_id}\n"
...
def open_event_wizard(app):
    ...
    # ── Autosave ─────────────────────────────────────────────────────
    _ev_autosave_path = os.path.join(tempfile.gettempdir(), "hoi4_cm_event_autosave.json")
    ...
    def _ev_save_state():
        ...
        try:
            ...
            for ev in events:
                data.append({
                    "eid": ev.eid, "etype": ev.etype, "title_text": ev.title_text,
                    "desc_text": ev.desc_text, "picture": ev.picture,
                    "major": ev.major, "fire_once": ev.fire_once,
                    "triggered": ev.triggered, "hidden": ev.hidden,
                    "mtth_days": ev.mtth_days, "mtth_months": ev.mtth_months,
                    "trigger_code": ev.trigger_code, "immediate": ev.immediate,
                    "options": ev.options,
                })
    ...
    def _ev_load_state():
        ...
        if not os.path.isfile(_ev_autosave_path):
            return False
        try:
            with open(_ev_autosave_path, "r", encoding="utf-8") as fp:
                data = json.load(fp)
            if not data:
                return False
            events.clear()
    ...
    def _find_gfx_file(gfx_name):
        ...
        if gfx_name in _ev_gfx_cache: return _ev_gfx_cache[gfx_name]
        path = None
        if MOD.loaded:
            ...
            if gfx_name in MOD.sprites:
                ...
                ev_dir = os.path.join(MOD.root, getattr(MOD,"path_event_pictures",
                    ...
                    if gfx_name.startswith(prefix):
                        ...
                        if os.path.isdir(ev_dir):
                            for rd, dirs, fnames in os.walk(ev_dir):
                                ...
                                for ext in (".dds",".tga",".png"):
                                    orig = lmap.get(stem + ext) or lmap.get((stem + ext).lower())
                                    if orig:
                                        path = os.path.join(rd, orig); break
                                if path: break
                        break
    ...
    def _read_image_size(path):
        if path in _ev_imgsize_cache: return _ev_imgsize_cache[path]
        result = None
        try:
    ...
    def _save_to_mod():
        ...
        if MOD.edit_loc_file and os.path.isfile(MOD.edit_loc_file):
            ...
            loc_file = os.path.join(mod_root, "localisation", "english",
                                     f"{ns}_l_english.yml")
        ...
        if MOD.edit_scripted_loc_file:
            ...
            if not eid:
                ...
                m3 = re.search(r"id\s*=\s*([\w\.]+)", _get_output_text() if callable(lambda: _get_output_text()) else "")
    ...
    def _browse_mod_events():
        ...
        if not MOD.loaded or not MOD.root:
            messagebox.showinfo("No Mod Loaded",
                "Load a mod first to browse existing events.", parent=win)
            return
        ev_dir = os.path.join(MOD.root, "events")
        if not os.path.isdir(ev_dir):
            messagebox.showinfo("Not Found",
                "No events/ directory found in mod.", parent=win)
            return
        files = sorted(_glob.glob(os.path.join(ev_dir, "*.txt")))
        if not files:
            messagebox.showinfo("No Files Found",
                "No .txt files found in events/.", parent=win)
            return
        ...
        def _do_import():
            ...
            try:
                with open(fp, encoding="utf-8-sig", errors="replace") as f:
                    raw = f.read()
                dupes = [m.group(1) for m in
                         _re2.finditer(r'\bids?\s*=\s*([^\s{}#\n]+)', raw)
                         if m.group(1) in existing_ids]
            except Exception:
                dupes = []
            if dupes:
                d_str = ", ".join(sorted(set(dupes))[:8])
                if not messagebox.askyesno("Duplicate IDs",
    ...
    def _import_txt(_path=None):
        path = _path or filedialog.askopenfilename(parent=win,
            filetypes=[("HOI4 Events","*.txt"),("All","*.*")],
        ...
        def _str_val(v, default=""):
            ...
            if isinstance(v, dict): return v.get("text", default)
    ...
    list_cv.bind("<MouseWheel>",   lambda e: list_cv.yview_scroll(int(-1*(e.delta/120)),"units"))
...
class ModContext:
    ...
    def __init__(self):
        ...
        self.md_money_system_file      = ""  # common/scripted_effects/00_money_system.txt
        self.md_money_scripted_loc_file= ""  # common/scripted_localisation/money_scripted_localization.txt
        self.md_money_yml_file         = ""  # localisation/english/MD_money_l_english.yml
    ...
    def _scan_md_money_files(self):
        ...
        r = self.root
        # 1. common/scripted_effects/00_money_system.txt
        p = os.path.join(r, "common", "scripted_effects", "00_money_system.txt")
        self.md_money_system_file = p if os.path.isfile(p) else ""
        # 2. common/scripted_localisation/ — find money_scripted_localization.txt (flexible name)
        ...
        if os.path.isdir(sloc_dir):
            for fn in os.listdir(sloc_dir):
                if "money" in fn.lower() and fn.endswith(".txt"):
                    ...
                    break
        # 3. localisation/english/MD_money_l_english.yml
...
class App(tk.Tk):
    ...
    def _build_toolbar_row2(self, toolbar):
        ...
        tk.Label(row2, text="y:", bg="#090d14", fg=TEXT_DIM, font=("Courier",9)).pack(side="right")
        _cfp_x_ent = tk.Entry(row2, textvariable=self._cfp_x_var, width=6,
                               bg=BG_CARD, fg=TEXT, insertbackground=BLUE,
                               font=("Courier",9), relief="flat",
                               highlightthickness=1, highlightbackground=BORDER_G)
    ...
    def _refresh_tree_meta_panel(self):
        ...
        if not hasattr(self, "_tree_meta_sf_box"): return
        for w in self._tree_meta_sf_box.winfo_children(): w.destroy()
        for w in self._tree_meta_jf_box.winfo_children(): w.destroy()
    ...
    def _build_sidebar(self, sb):
        ...
        tk.Label(sf_row, text="shared_focus:", bg=BG_PANEL, fg=TEXT_DIM,
                 font=("Helvetica",7), anchor="w", width=14).pack(side="left")
        self._tree_meta_sf_box = tk.Frame(sf_row, bg=BG_CARD,
                                          highlightthickness=1, highlightbackground=BORDER_G)
        self._tree_meta_sf_box.pack(side="left", fill="x", expand=True)
        # Joint focuses row
        jf_row = tk.Frame(meta, bg=BG_PANEL); jf_row.pack(fill="x", padx=4, pady=(1,4))
        tk.Label(jf_row, text="joint_focus:", bg=BG_PANEL, fg=TEXT_DIM,
                 font=("Helvetica",7), anchor="w", width=14).pack(side="left")
        self._tree_meta_jf_box = tk.Frame(jf_row, bg=BG_CARD,
                                          highlightthickness=1, highlightbackground=BORDER_G)
    ...
    def _build_sidebar_props(self, p, _make_scroll_panel):
        ...
        self._add_offset_btn = tk.Button(self._sb_frm, text="＋  Add Offset",
                  ...
                  highlightthickness=1, highlightbackground="#06b6d4")
    ...
    def _refresh_offsets(self, f=None):
        ...
        if f is None: f = self.selected
        for w in self._offset_box.winfo_children(): w.destroy()
        ...
        for i, off in enumerate(offsets):
            row = tk.Frame(self._offset_box, bg=BG_CARD, highlightthickness=1,
                           highlightbackground=BORDER_G, pady=2)
            row.pack(fill="x", pady=2)
            # X / Y row + delete button
            xy_row = tk.Frame(row, bg=BG_CARD); xy_row.pack(fill="x", padx=4)
            tk.Label(xy_row, text="x:", bg=BG_CARD, fg=TEXT_DIM,
                     font=("Helvetica", 9), width=2).pack(side="left")
            x_var = tk.StringVar(value=str(off.get("x", 0)))
            tk.Entry(xy_row, textvariable=x_var, bg=BG_PANEL, fg=TEXT,
                     font=("Helvetica", 9), relief="flat", width=5,
                     highlightthickness=1, highlightbackground=BORDER_G).pack(side="left", ipady=2, padx=(0, 6))
            tk.Label(xy_row, text="y:", bg=BG_CARD, fg=TEXT_DIM,
                     font=("Helvetica", 9), width=2).pack(side="left")
            y_var = tk.StringVar(value=str(off.get("y", 0)))
            tk.Entry(xy_row, textvariable=y_var, bg=BG_PANEL, fg=TEXT,
                     font=("Helvetica", 9), relief="flat", width=5,
                     highlightthickness=1, highlightbackground=BORDER_G).pack(side="left", ipady=2)
            del_btn = tk.Button(xy_row, text="✕", bg=BG_CARD, fg=RED, relief="flat",
                                font=("Georgia", 8), cursor="hand2", padx=3)
            del_btn.config(command=lambda idx=i: self._del_offset(idx))
            del_btn.pack(side="right")
            # trigger raw block
            tk.Label(row, text="trigger = {", bg=BG_CARD, fg=TEXT_DIM,
                     font=("Courier", 8), anchor="w").pack(fill="x", padx=4)
            trig_text = tk.Text(row, bg=BG_PANEL, fg=TEXT, insertbackground=BLUE,
                                font=("Courier", 8), height=3, relief="flat",
                                wrap="none", highlightthickness=1, highlightbackground=BORDER_G)
    ...
    def _draw_lines(self):
        ...
        for idx, (etype, aid, bid, cross) in enumerate(edges):
            ...
            if etype == "arr":
                ...
                arr_dash = (max(4,int(6*self.zoom)), max(3,int(4*self.zoom))) if cross else ()
    ...
    def _draw_focus(self, f):
        ...
        border_col = FC_SEL_BD if sel else ("#00e5ff" if msel else (BLUE if con else (ORANGE if mut else base_border)))
        ...
        state_key = (round(cx,1), round(cy,1), round(h,1), sel, msel, con, mut, label_text, ico_size, lbl_size, getattr(f,"gfx",""), tree_idx, has_offsets)
        ...
        if not f._items:
            ...
            badge    = cv.create_text(0, 0, text="",
                                      font=("Helvetica", max(5, int(5*z)), "bold"),
                                      fill="#000000",                  tags=("focus",tag))
            off_ind  = cv.create_text(0, 0, text="",
                                      font=("Helvetica", max(4, int(4*z)), "bold"),
                                      fill="#06b6d4",                  tags=("focus",tag))
            f._items = [shadow,mat,box_rect,rv0,rv1,rv2,rv3,glow,ico,img_item,lbl_bg,lbl,badge,off_ind]
        ...
        shadow,mat,box_rect,rv0,rv1,rv2,rv3,glow,ico,img_item,lbl_bg,lbl,badge,off_ind = f._items
        ...
        if badge_txt and z >= 0.4:
            cv.itemconfig(badge, text=badge_txt,
                          font=("Helvetica", badge_sz, "bold"),
                          fill=tree_col, state="normal")
        else:
        ...
        if has_offsets and z >= 0.3:
            cv.itemconfig(off_ind, text="⊕",
                          font=("Helvetica", off_sz, "bold"),
                          fill="#06b6d4", state="normal")
        else:
    ...
    def _on_mod_loaded(self, pw, root):
        ...
        for _c in _app_img_caches: _c.clear()
    ...
    def _show_post_load_prompt(self):
        ...
        _body_cv = tk.Canvas(_body_outer, bg=BG_DARK, highlightthickness=0,
                             yscrollcommand=_body_vsb.set)
        ...
        def _confirm():
            ...
            if MOD.edit_focus_file:  parts.append("focus: " + os.path.basename(MOD.edit_focus_file))
    ...
    def _foc_en(self,fid):
        ...
        if offs:
            off_strs = [f"x={o['x']} y={o['y']} [{o.get('trigger','').strip()[:30]}]" for o in offs]
    ...
    def _apply_focus_code(self, f, new_code):
        ...
        try:
            def _extract_block(text, start):
                ...
                while i < len(text):
                    if text[i] == "{": depth += 1
                        ...
                        depth -= 1
                        if depth == 0: return text[start+1:i].strip(), i
                    i += 1
            ...
            for flag, attr in [("cancel_if_invalid",       "cancel_if_invalid"),
                ...
                if m: setattr(f, attr, m.group(1) == "yes")

            m = _re.search(r"\bsearch_filters\s*=\s*\{([^}]*)\}", new_code)
            if m: f.search_filters = m.group(1).strip()
            ...
            if raw_ai: f.ai_will_do_raw = raw_ai
            ...
            for _om in _re.finditer(r'\boffset\s*=\s*\{', new_code):
                _os = _om.end() - 1; _od = 0; _oi = _os
                while _oi < len(new_code):
                    if new_code[_oi] == '{': _od += 1
                    elif new_code[_oi] == '}':
                        _od -= 1
                        if _od == 0: break
                    _oi += 1
            ...
            if self.selected and self.selected.id == f.id:
                self._populate(f)
                for tv, attr in [(self._fv_avail,  "available_cond"),
    ...
    def _pick_prereq(self):
        ...
        sv = tk.StringVar()
        se = tk.Entry(win, textvariable=sv, bg=BG_PANEL, fg=TEXT,
        ...
        frm = tk.Frame(win, bg=BG_DARK); frm.pack(fill="both", expand=True, padx=10, pady=4)
        sb  = tk.Scrollbar(frm, orient="vertical")
        lb  = tk.Listbox(frm, bg=BG_PANEL, fg=TEXT, selectbackground=BLUE,
                         selectforeground=BG_DARK, font=("Courier", 9),
                         relief="flat", highlightthickness=0,
                         selectmode=tk.EXTENDED, yscrollcommand=sb.set,
                         activestyle="dotbox")
        ...
        def _populate(filter_text=""):
            lb.delete(0, "end"); _id_map.clear()
            ft = filter_text.lower().strip()
            for f in sorted_cands:
                if ft and ft not in f.name.lower(): continue
                t_idx = getattr(f, "tree_idx", 0)
                tree_badge = ""
                if t_idx > 0:
        ...
        def _show_tip(e):
            idx = lb.nearest(e.y)
            if idx < 0 or idx not in _id_map: return
            ...
            tk.Label(t, text=f.name, bg="#1e3a5f", fg=TEXT,
                     font=("Courier",9), padx=6, pady=3, relief="flat",
                     bd=1, highlightbackground=BLUE, highlightthickness=1).pack()
            _tip_win[0] = t
        def _hide_tip(e):
            if _tip_win[0]:
                try: _tip_win[0].destroy()
        ...
        # AND/OR explanation
        expl = tk.Frame(win, bg="#0d1525", relief="flat",
                        highlightthickness=1, highlightbackground=BORDER_G)
        ...
        btn_row = tk.Frame(win, bg=BG_DARK); btn_row.pack(fill="x", padx=10, pady=8)
        ...
        def _confirm_or():
            ...
            self._refresh_prereqs(); self._draw_lines(); win.destroy()
    ...
    def _end_connect(self):
        self.conn_src = None
        if self._temp_line:
            self.cv.delete(self._temp_line); self._temp_line = None
    ...
    def _import_drawio(self):
        ...
        code_lines.append(f"\t}}")
        code_lines.append("")
        code_lines.append(f"\tcontinuous_focus_position = {{ x = 0 y = 1700 }}")
        code_lines.append("")
        ...
        for cid in sorted_cids:
            ...
            code_lines.append(f"\t\ticon = GFX_goal_generic_political_pressure  # TODO: replace with real icon")
            code_lines.append("")
            ...
            code_lines.append(f"\t\t# available = {{ }}")
            code_lines.append(f"\t\t# bypass = {{ }}")
            code_lines.append(f"\t\t# cancel = {{ }}")
            code_lines.append("")
            code_lines.append(f"\t\tcompletion_reward = {{")
            code_lines.append(f'\t\t\tlog = "[GetDateText]: [This.GetName]: focus {fid} executed"')
            code_lines.append(f"\t\t\t# TODO: add effects")
            code_lines.append(f"\t\t}}")
            code_lines.append("")
            code_lines.append(f"\t\tai_will_do = {{")
            code_lines.append(f"\t\t\tfactor = 1")
            code_lines.append(f"\t\t}}")
            code_lines.append(f"\t}}")
            code_lines.append("")
    ...
    def _import_txt(self):
        ...
        if not focuses_data:
            i = 0
            while i < len(tokens):
                if tokens[i] in ("focus", "shared_focus") and \
        ...
        for rf in focuses_data:
            ...
            f.available_cond = _raw_rewards.get((fid_str,'available'),
                                _block_to_str(rf.get("available",{})))
            f.bypass_cond    = _raw_rewards.get((fid_str,'bypass'),
    ...
    def _get_tree_badge(self, tree_idx):
        ...
        tt = info["type"]
        if tt == "shared":
            n = sum(1 for t in self._extra_trees[:tree_idx - 1] if t["type"] == "shared")
            return ("S" if n == 0 else f"S{n + 1}"), _SHARED_COLS[n % len(_SHARED_COLS)]
        else:
            n = sum(1 for t in self._extra_trees[:tree_idx - 1] if t["type"] == "joint")
    ...
    def _draw_canvas_legend(self):
        ...
        for idx, et in enumerate(self._extra_trees, start=1):
            ...
            items.append((f"■ [{badge}] {et['type'].capitalize()}: {et['tree_id']}", col))
        items.append(("· · ·  cross-tree prereq", "#94a3b8"))
        for lbl, col in reversed(items):
            cv.create_text(x, y, text=lbl, fill=col, anchor="sw",
                           font=("Helvetica", 8), tags="legend")
            y -= 14
    ...
    def _draw_cfp_markers(self):
        ...
        def _draw_box(gx, gy, color, label):
            ...
            cv.create_rectangle(cx - h, cy - h, cx + h, cy + h,
                                 outline=color, fill=color, stipple="gray12",
                                 width=lw,
                                 dash=(max(4, int(6 * z)), max(3, int(4 * z))),
                                 tags="cfp_marker")
            if z >= 0.3:
                cv.create_text(cx, cy, text=label, fill=color, anchor="center",
                               font=("Helvetica", font_sz, "bold"),
                               width=max(60, int(h * 1.8)),
                               tags="cfp_marker")

        # Main tree CFP
        if getattr(self, "_cfp_x", None) is not None and \
                getattr(self, "_cfp_y", None) is not None:
            _draw_box(self._cfp_x / XGRID, self._cfp_y / YGRID,
                      "#22d3ee", "Continuous\nFocus Position")
        ...
        for idx, et in enumerate(getattr(self, "_extra_trees", []), start=1):
            if et.get("cfp_x") is not None and et.get("cfp_y") is not None:
                ...
                if tree_type == "joint":
                    ...
                    col = "#f59e0b"
                _draw_box(et["cfp_x"] / XGRID, et["cfp_y"] / YGRID,
                          col, f"Continuous\nFocus Position\n[{badge}]")
    ...
    def _load_extra_tree(self, tree_type):
        ...
        import re as _re2
        sf_refs = _re2.findall(r'\bshared_focus\s*=\s*(\S+)', txt)
        jf_refs = _re2.findall(r'\bjoint_focus\s*=\s*(\S+)', txt)

        def _extract_raw_block(source, key):
            import re
        ...
        import re as _re
        _raw_rewards = {}
        # Match shared_focus, joint_focus, and plain focus blocks
        for _fm in _re.finditer(r'\b(?:shared_focus|joint_focus|focus)\s*=\s*\{', txt):
            _fs = _fm.end() - 1; _d = 0; _fi = _fs
            while _fi < len(txt):
                if txt[_fi] == '{': _d += 1
                elif txt[_fi] == '}':
                    _d -= 1
            ...
            for _om in _re.finditer(r'\boffset\s*=\s*\{', _fblock):
                ...
                _otrig = _extract_raw_block(_oinner, 'trigger')
                _offsets.append({"x": _ox, "y": _oy, "trigger": _otrig})
            if _offsets: _raw_rewards[(_fid, '_offsets')] = _offsets
            # Preserve joint_trigger as extra raw text (offset now stored separately)
            _jt = _extract_raw_block(_fblock, 'joint_trigger')
            if _jt: _raw_rewards[(_fid, '_joint_extra')] = f"joint_trigger = {{\n{_jt}\n}}"
        ...
        focuses_data = []
        tree_name = "imported_tree"
        cfp_x = cfp_y = None
        country_tag = "TAG"
        i = 0
        while i < len(tokens):
            if tokens[i] == "focus_tree" and i + 1 < len(tokens) and tokens[i + 1] == "=":
                block, i = _parse_blk(tokens, i + 2)
                tree_name = block.get("id", "imported_tree")
                _cfp_blk = block.get("continuous_focus_position", {})
                if isinstance(_cfp_blk, dict):
                ...
                if isinstance(_country_blk, dict):
                    _mod_blk = _country_blk.get("modifier", {})
                    if isinstance(_mod_blk, dict):
                        _tag = (_mod_blk.get("original_tag") or _mod_blk.get("tag") or "").upper().strip()
                        if _tag: country_tag = _tag
                raw_focuses = block.get("focus", [])
                if isinstance(raw_focuses, dict): raw_focuses = [raw_focuses]
                for rf in raw_focuses:
                    if isinstance(rf, dict): focuses_data.append(rf)
        ...
        # Fallback: handle files without a focus_tree = { } wrapper.
        # Handles shared_focus and joint_focus top-level blocks.
        if not focuses_data:
            i = 0
            while i < len(tokens):
                if tokens[i] in ("focus", "shared_focus", "joint_focus") and \
                        i + 1 < len(tokens) and tokens[i + 1] == "=" and \
                        i + 2 < len(tokens) and tokens[i + 2] == "{":
                    blk, i = _parse_blk(tokens, i + 2)
                    if isinstance(blk, dict) and "id" in blk:
                        focuses_data.append(blk)
                else:
                    i += 1
            if focuses_data and tree_name == "imported_tree":
                tree_name = os.path.splitext(os.path.basename(path))[0]

        if not focuses_data:
            import re as _re_diag
            _found = [kw for kw in ("focus_tree", "focus", "shared_focus", "joint_focus")
                      if _re_diag.search(rf'\b{kw}\s*=\s*\{{', txt)]
            _detail = (f"Blocks found: {', '.join(_found)} — none had valid focus IDs."
                       if _found else "No recognized block types found.")
            messagebox.showwarning("Load Tree",
                f"No focus data found in:\n{os.path.basename(path)}\n\n{_detail}")
            return

        # Assign tree_idx for this extra tree
        ...
        tree_info = {
            ...
            "cfp_x": cfp_x,
            "cfp_y": cfp_y,
            "shared_focuses": sf_refs,
            "joint_focuses":  jf_refs,
        ...
        # Auto-register tree_id in main tree's shared/joint reference list
        if tree_type == "shared" and tree_name not in self._shared_focuses:
            self._shared_focuses.append(tree_name)
        elif tree_type == "joint" and tree_name not in self._joint_focuses:
            self._joint_focuses.append(tree_name)
        self._refresh_tree_meta_panel()
        ...
        for rf in focuses_data:
            ...
            raw_pos[fid_str] = (gx, gy, rel_id)
            f = Focus(gx, gy)
            f._raw_gx = gx; f._raw_gy = gy  # original file coords, preserved for export
            f.tree_idx = tree_idx
            f.name = fid_str
            if rel_id:
                f.relative_position_id = rel_id
        ...
        # Apply country-tag-matching offsets to raw_pos BEFORE resolution so that
        # all relative-positioned children naturally inherit the shift.
        _active_tag = getattr(self, '_tree_country_tag', '').upper().strip()
        if _active_tag:
            for _fid_s in list(raw_pos.keys()):
                for _off in _raw_rewards.get((_fid_s, '_offsets'), []):
                    _trig = _off.get('trigger', '')
                    if _re.search(r'\boriginal_tag\s*=\s*' + _re.escape(_active_tag) + r'\b', _trig):
                        _rx, _ry, _rel = raw_pos[_fid_s]
                        raw_pos[_fid_s] = (_rx + _off['x'], _ry + _off['y'], _rel)
                        break  # apply only the first matching offset per focus

        # Resolve relative positions — looks within this tree first, then cross-tree
        def _resolve_abs(name, visited=None):
            if visited is None: visited = set()
            if name not in raw_pos: return (0, 0)
            if name in visited: return raw_pos[name][:2]
            visited.add(name)
            rx, ry, rel = raw_pos[name]
            if rel is None: return (rx, ry)
            if rel in raw_pos:
                bx, by = _resolve_abs(rel, visited)
                return (bx + rx, by + ry)
            # Cross-tree: anchor is in a different loaded tree (e.g. main tree focus)
            for _cf in self.focuses.values():
                if _cf.name == rel:
                    return (_cf.x + rx, _cf.y + ry)
            return (rx, ry)

        for fid_str, fid in name_to_id.items():
            ax, ay = _resolve_abs(fid_str)
            self.focuses[fid].x = ax
            self.focuses[fid].y = ay

        # Pass 2: link prerequisites and mutex (cross-tree lookups included)
        all_name_to_id = {f.name: f.id for f in self.focuses.values()}
        for rf in focuses_data:
            fid_str = rf.get("id", "")
            if fid_str not in name_to_id: continue
            fid = name_to_id[fid_str]
            ...
            for pblock in prereqs:
                ...
                if isinstance(pf, str): pf = [pf]
                for pname in pf:
                    if pname in all_name_to_id:
                        group_fids.append(all_name_to_id[pname])
                if group_fids: f.prereqs.append(group_fids)
            mutex = rf.get("mutually_exclusive", [])
            if isinstance(mutex, dict): mutex = [mutex]
            for mblock in mutex:
        ...
        messagebox.showinfo("Loaded",
            f"Loaded {len(focuses_data)} focuses from {tree_type} tree:\n"
    ...
    def _refresh_loaded_trees_panel(self):
        ...
        for idx, et in enumerate(self._extra_trees, start=1):
            ...
            row = tk.Frame(self._loaded_trees_inner, bg=BG_CARD,
                           highlightthickness=1, highlightbackground=BORDER_G)
            row.pack(fill="x", padx=4, pady=2)
            tk.Label(row, text=f" [{badge_txt}]", bg=badge_col, fg="#000000",
                     font=("Courier", 8, "bold"), padx=4, pady=2).pack(side="left")
            info_f = tk.Frame(row, bg=BG_CARD); info_f.pack(side="left", fill="x", expand=True)
            tk.Label(info_f, text=et["tree_id"],
            ...
            tk.Button(btn_f, text="✕", command=lambda i=idx: self._unload_extra_tree(i),
                      bg=BG_CARD, fg=TEXT_DIM, font=("Helvetica", 8), relief="flat",
                      padx=4, pady=1, cursor="hand2").pack(pady=(0, 4))

    def _export_extra_tree(self, tree_idx):
        """Export a single extra (shared/joint) tree to its source file."""
        if tree_idx <= 0 or tree_idx > len(self._extra_trees): return
        info = self._extra_trees[tree_idx - 1]
        ...
        cfp_x = info.get("cfp_x"); cfp_y = info.get("cfp_y")
        is_joint = info.get("type") == "joint"

        def _write_focus_body(f, out, indent, strip_effect_tab=False):
            """Write the shared body of a focus/joint_focus block (fields after id/icon/x/y/cost).

            strip_effect_tab: _render_effect outputs with 3-tab indent; set True for joint trees
            where completion_reward is only 1 level deep (so effects need 2-tab indent).
            """
            T = indent; T2 = indent + "\t"
            # Use original file coords (not offset-applied canvas coords) for export
            gx = getattr(f, '_raw_gx', f.x); gy = getattr(f, '_raw_gy', f.y)
            rel_id = getattr(f, "relative_position_id", None)
            if rel_id and any(foc.name == rel_id for foc in self.focuses.values()):
                dx = getattr(f, "_rel_dx", None); dy = getattr(f, "_rel_dy", None)
                if dx is None or dy is None:
                    parent = next((foc for foc in self.focuses.values() if foc.name == rel_id), None)
                    dx = gx - parent.x if parent else gx
                    dy = gy - parent.y if parent else gy
                out.append(f"{T}x = {dx}"); out.append(f"{T}y = {dy}")
                out.append(f"{T}relative_position_id = {rel_id}")
            else:
                out.append(f"{T}x = {gx}"); out.append(f"{T}y = {gy}")
            for _off in getattr(f, "offsets", []):
                out.append(f"{T}offset = {{")
                out.append(f"{T}\tx = {_off['x']}")
                out.append(f"{T}\ty = {_off['y']}")
                if _off.get("trigger", "").strip():
                    out.append(f"{T}\ttrigger = {{")
                    for _ln in _off["trigger"].strip().splitlines():
                        out.append(f"{T}\t\t{_ln.strip()}")
                    out.append(f"{T}\t}}")
                out.append(f"{T}}}")
            out.append(f"{T}cost = {f.cost}")
            # Joint-specific preserved fields (joint_trigger only now — offset stored in f.offsets)
            ...
            if f.prereqs:
                for grp in f.prereqs:
                    ...
                    inner = " ".join(f"focus = {self.focuses[p].name}" for p in valid)
                    out.append(f"{T}prerequisite = {{ {inner} }}")
            if f.mutex:
                for mid in f.mutex:
                    if mid in self.focuses:
                        out.append(f"{T}mutually_exclusive = {{ focus = {self.focuses[mid].name} }}")
            sf = getattr(f, "search_filters", "").strip()
            if sf: out.append(f"{T}search_filters = {{ {sf} }}")
            for cond_key, cond_attr in [("available", "available_cond"), ("bypass", "bypass_cond"), ("cancel", "cancel_cond")]:
                cond = getattr(f, cond_attr, "").strip()
                if cond:
                    out.append(f"{T}{cond_key} = {{")
            ...
            if f.effects:
                for eff in f.effects:
                    eff_text = self._render_effect(eff)
                    if strip_effect_tab:
                        # _render_effect uses \t\t\t base indent; joint needs \t\t
                        eff_text = "\n".join(
        ...
        if is_joint or not had_wrapper:
            ...
            for f in focuses_in_tree:
                ...
                _write_focus_body(f, out, "\t", strip_effect_tab=True)
                out.append("}")
                out.append("")
        else:
    ...
    def _load_all_trees(self):
        ...
        sb = tk.Scrollbar(frm, orient="vertical")
        sb.pack(side="right", fill="y")
        list_canvas = tk.Canvas(frm, bg=BG_DARK, highlightthickness=0,
        ...
        list_canvas.bind("<Configure>",
                         lambda e: list_canvas.itemconfig(inner_win, width=e.width))
        ...
        for fp in all_files:
            ...
            if m:
                n = int(m.group(1))
                if n >= 5: def_type = "main"
                else: def_type = "shared"
            ...
            row_f = tk.Frame(inner, bg=BG_CARD if not already else "#1a1f2e",
                             highlightthickness=1, highlightbackground=BORDER_G)
            row_f.pack(fill="x", padx=2, pady=1)

            chk = tk.Checkbutton(row_f, variable=chk_var,
                                 bg=row_f.cget("bg"), fg=TEXT,
            ...
            name_lbl = tk.Label(row_f, text=fname,
                                bg=row_f.cget("bg"),
            ...
            for lbl, val, col in [("Main", "main", TEXT_DIM),
                ...
                tk.Radiobutton(type_f, text=lbl, variable=type_var, value=val,
                               ...
                               selectcolor=BG_DARK, activebackground=row_f.cget("bg"),
                               font=("Helvetica", 8), relief="flat").pack(side="left")
            if already:
                for w in type_f.winfo_children(): w.config(state="disabled")
        ...
        def _sel_all():
            for fp, cv2, tv, already in rows:
                if not already and tv.get() != "main":
                    cv2.set(True)
        def _desel_all():
            for _, cv2, _, _ in rows: cv2.set(False)
        def _sel_shared():
            for _, cv2, tv, already in rows:
                if not already: cv2.set(tv.get() == "shared")
        def _sel_joint():
            for _, cv2, tv, already in rows:
    ...
    def _load_extra_tree_from_path(self, path, tree_type):
        ...
        # Fallback: top-level focus/shared_focus/joint_focus blocks (no focus_tree wrapper)
        if not focuses_data:
            i = 0
            while i < len(tokens):
                if tokens[i] in ("focus", "shared_focus", "joint_focus") and \
        ...
        if not focuses_data:
            raise ValueError(f"No focus data found in {os.path.basename(path)}")

        tree_idx = len(self._extra_trees) + 1
        tree_info = {
            "type": tree_type, "file_path": path, "tree_id": tree_name,
            "cfp_x": cfp_x, "cfp_y": cfp_y, "shared_focuses": sf_refs,
            "joint_focuses": jf_refs, "country_tag": country_tag,
            "had_wrapper": had_wrapper, "focus_ids": set(),
        }
        self._extra_trees.append(tree_info)
        if tree_type == "shared" and tree_name not in self._shared_focuses:
            self._shared_focuses.append(tree_name)
        ...
        for rf in focuses_data:
            ...
            f = Focus(gx, gy)
            f._raw_gx = gx; f._raw_gy = gy  # original file coords, preserved for export
            f.tree_idx = tree_idx; f.name = fid_str
            if rel_id: f.relative_position_id = rel_id; f._rel_dx = gx; f._rel_dy = gy
            f.gfx = rf.get("icon", "GFX_goal_generic_political_pressure")
            try:
            ...
            if raw_rw:
                ...
                f.effects = []
            self.focuses[f.id] = f; name_to_id[fid_str] = f.id
            tree_info["focus_ids"].add(f.id)
        ...
        def _resolve_abs(name, visited=None):
            ...
            if rel in raw_pos:
                bx, by = _resolve_abs(rel, visited)
                return (bx + rx, by + ry)
            # Cross-tree: anchor is in a different loaded tree (e.g. main tree)
            for _cf in self.focuses.values():
                if _cf.name == rel:
                    return (_cf.x + rx, _cf.y + ry)
            return (rx, ry)
        ...
        for fid_str, fid in name_to_id.items():
            ax, ay = _resolve_abs(fid_str)
            self.focuses[fid].x = ax; self.focuses[fid].y = ay

        all_name_to_id = {f.name: f.id for f in self.focuses.values()}
        for rf in focuses_data:
            fid_str = rf.get("id", "")
            if fid_str not in name_to_id: continue
            fid = name_to_id[fid_str]; f = self.focuses[fid]
            prereqs = rf.get("prerequisite", [])
            if isinstance(prereqs, dict): prereqs = [prereqs]
            for pblock in prereqs:
                if not isinstance(pblock, dict): continue
    ...
    def _render_effect(self, eff):
        ...
        if t in ("set_temp_variable_to_random", "randomize_temp_variable"):
            return f"{I}{t} = {{ {g('var','my_temp_var')} = {{ max = {g('max','10')} }} }}"
        ...
        if t == "md_build_state":
            ...
            return (f"{I}set_temp_variable = {{ treasury_change = {tchange} }}\n"
                    f"{I}modify_treasury_effect = yes\n"
                    f"{I}{state} = {{\n"
                    f"{I}\t{effect} = yes\n"
                    f"{I}}}")
    ...
    def _apply_md_additional_income(self, idea_id, variable_name, amount, tooltip_key, formula_type="fixed"):
        ...
        if not money_sys:
            money_sys = os.path.join(MOD.root, "common", "scripted_effects", "00_money_system.txt")
        step1_done = False
        if os.path.isfile(money_sys):
            try:
                with open(money_sys, "r", encoding="utf-8-sig", errors="replace") as fp:
                    sys_text = fp.read()
                # Check if already present
                if f"has_idea = {idea_id}" in sys_text and f"{variable_name}" in sys_text:
                    saved.append(f"00_money_system.txt — already contains '{idea_id}' entry (skipped)")
                    step1_done = True
                else:
                    ...
                    m = _re.search(r'calculate_additional_income_rate\s*=\s*\{', sys_text)
                    if m:
                        # Find its closing brace
                        depth = 0; i = m.end() - 1
                        while i < len(sys_text):
                            if sys_text[i] == '{': depth += 1
                            elif sys_text[i] == '}':
                                depth -= 1
                                if depth == 0: break
                            i += 1
                        ...
                        errs.append(f"⚠ Could not find 'calculate_additional_income_rate' in {os.path.basename(money_sys)}. Insert manually.")
            except Exception as e:
                errs.append(f"❌ 00_money_system.txt: {e}")
        else:
            errs.append(f"⚠ 00_money_system.txt not found at expected path. Insert manually:\n"
                        f"  common/scripted_effects/00_money_system.txt → calculate_additional_income_rate")
        ...
        try:
            ...
            if os.path.isfile(sloc):
                with open(sloc, "r", encoding="utf-8-sig", errors="replace") as fp:
                    existing_sloc = fp.read()
            summary_name = f"additional_income_summary_{idea_id}"
            if summary_name in existing_sloc:
                saved.append(f"money_scripted_localization.txt — '{summary_name}' already present (skipped)")
            else:
                defined_text = (
                    f"\ndefined_text = {{\n"
                    f"\tname = {summary_name}\n"
        ...
        try:
            ...
            if os.path.isfile(yml):
                with open(yml, "r", encoding="utf-8-sig", errors="replace") as fp:
                    yml_text = fp.read()
            if summary_token in yml_text:
                saved.append(f"MD_money_l_english.yml — '{summary_token}' already present (skipped)")
            else:
                ...
                if m2:
                    new_yml = yml_text[:m2.start(3)] + f"\\n{summary_token}" + yml_text[m2.start(3):]
                    with open(yml, "w", encoding="utf-8-sig") as fp:
                        fp.write(new_yml)
                    rel = os.path.relpath(yml, MOD.root)
                    saved.append(f"✅ {rel}  — appended '{summary_token}' to {tooltip_loc_key}")
                else:
                    ...
                    rel = os.path.relpath(yml, MOD.root)
                    saved.append(f"✅ {rel}  — appended new '{tooltip_loc_key}' entry (key not found, added at end)")
        except Exception as e:
    ...
    def _apply_md_visibility(self):
        ...
        if hasattr(self, "_eff_cat"):
            ...
            if hasattr(self, "_eff_cat_menu_widget") and self._eff_cat_menu_widget.winfo_exists():
                menu = self._eff_cat_menu_widget["menu"]
                menu.delete(0, "end")
                for cat in EFFECT_CATS:
                    menu.add_command(label=cat,
                                     command=lambda c=cat: (self._eff_cat.set(c),
                                                            self._rebuild_eff_dd()))
            self._rebuild_eff_dd()
    ...
    def _toggle_minimap(self):
        ...
        if self._mm_visible:
            ...
            if not hasattr(self, "_mm_canvas") or not self._mm_canvas.winfo_exists():
                mm = tk.Canvas(self.cv, bg="#070c15",
                               highlightthickness=1,
                               highlightbackground=BORDER_G,
                               cursor="crosshair")
                self._mm_canvas = mm
                ...
                # Close button
                close_lbl = tk.Label(mm, text="×", bg="#070c15", fg=TEXT_DIM,
            ...
            self._mm_canvas.place(relx=1.0, rely=1.0, anchor="se",
                                  x=-8, y=-8, width=220, height=150)
    ...
    def _draw_minimap(self):
        ...
        if not getattr(self, "_mm_visible", False): return
        if not hasattr(self, "_mm_canvas"): return
        try:
            mm = self._mm_canvas
        ...
        for et in getattr(self, "_extra_trees", []):
            if et.get("cfp_x") is not None: all_xs.append(et["cfp_x"] / XGRID)
            if et.get("cfp_y") is not None: all_ys.append(et["cfp_y"] / YGRID)

        min_x = min(all_xs) - 1; max_x = max(all_xs) + 1
        min_y = min(all_ys) - 1; max_y = max(all_ys) + 1
        ...
        self._mm_min_x = min_x; self._mm_min_y = min_y
        self._mm_ox = ox;       self._mm_oy = oy
            ...
            for grp in f.prereqs:
                for pid in grp:
                    if pid in self.focuses:
                        ...
                        mm.create_line(fx, fy, px, py,
                                       fill="#1e3048", width=1, tags="mm_content")
        ...
        for f in self.focuses.values():
            ...
            if t_idx == 0: col = "#475569"
            mm.create_rectangle(mx - dot, my - dot, mx + dot, my + dot,
                                 fill=col, outline="", tags="mm_content")

        # Draw CFP markers as tiny outlined squares
        def _mm_cfp(gx, gy, col):
            mx, my = g2mm(gx, gy)
            s = max(3, int(scale * 0.6))
            mm.create_rectangle(mx - s, my - s, mx + s, my + s,
                                 outline=col, fill="", width=1,
                                 dash=(2, 2), tags="mm_content")
        ...
        mm.create_text(4, MM_H - 4, text="M — minimap",
                       fill="#1e3048", anchor="sw",
                       font=("Helvetica", 7), tags="mm_content")

    def _mm_click(self, e):
        """Pan the main canvas to the position clicked on the minimap."""
        if not hasattr(self, "_mm_scale") or not self._mm_scale: return
    ...
    def _export(self):
        ...
        country_tag = tag_match.group(1) if tag_match else getattr(self, "_tree_country_tag", "TAG")
        ...
        if MOD.edit_loc_file and os.path.isfile(MOD.edit_loc_file):
            ...
            if _mod_root is None:
                ...
                for _ in range(5):
                    ...
                    if (os.path.isdir(os.path.join(_candidate, "common")) and
                            os.path.isdir(os.path.join(_candidate, "localisation"))):
                        _mod_root = _candidate
                        break
            if _mod_root:
                loc_path = os.path.join(_mod_root, "localisation", "english", loc_filename)
            else:
